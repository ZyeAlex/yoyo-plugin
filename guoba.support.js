...
export function supportGuoba() {
  ...
  const group = [
    ...
    {
      field: "config.auditInclude",
      label: "加群审核配置",
      bottomHelpMessage: "配置进群审核",
      component: "GSubForm",
      componentProps: {
        multiple: true,
        modalProps: {
        ...
        schemas: [
          ...
          },
          {
            field: "answer",
            label: "答案",
            bottomHelpMessage: '入群问题答案 (正则匹配，忽略大小写，如果不会正则可以将需求输入AI生成)',
            required: true,
            component: "Input",
          }
        ]
    ...
    {
      field: 'config.authLevel',
      label: '加群审核等级',
      bottomHelpMessage: '低于该等级插件将不予审核，由管理员自行定夺(推荐32~48级)',
      component: 'InputNumber',
      required: false,
      componentProps: {
        min: 0,
    ...
    {
      field: 'config.refuseLevel',
      label: '加群拒绝等级',
      bottomHelpMessage: '低于该等级插件将自动拒绝 (用于过滤⭐️等级小号，推荐2~8级)',
      component: 'InputNumber',
      required: false,
      componentProps: {
        min: 0,
    ...
    {
      field: "config.excludeText",
      label: "加群词过滤",
      bottomHelpMessage: '过滤广告、人机入群话术（正则匹配，忽略大小写）',
      component: "GTags",
      componentProps: {
        allowAdd: true,
        allowDel: true,
        showPrompt: true,
        promptProps: {
          content: "请输入过滤词 (正则匹配，忽略大小写，如果不会正则可以将需求输入AI生成)",
          placeholder: "请输入过滤正则",
          okText: "添加过滤",
          rules: [{ required: true, message: "需要填写过滤正则才行哦~" }]
        }
      }
    },
