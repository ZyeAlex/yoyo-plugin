...
export const tool_functions = {
    ...
    get_infos: async () => {
        return JSON.stringify({
            '角色': Array.from(Object.keys(game.heroIds)),
            '角色的别名(外号)': Object.entries(game.nicknames).map(([id, nick]) => ({ [game.heros[id].name]: nick })),
            '奇波': Array.from(Object.keys(game.petIds))
        })
    ...
    get_info: async ({ name, type }) => {
        switch (type) {
            case 'hero':
                return JSON.stringify(
                    game.heros[game.getHeroId(name)]
                )
            case 'kibo':
                return JSON.stringify(
                    game.pets[game.petIds[name]]
                )
        }
    },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "prompt": {
                        "type": "string",
                        "description": "图片生成的提示词，描述图片画风、内容、元素、氛围等，请根据用户需求自行丰富提示词"
                    },
                    "size": {
                        "type": "string",
                        "description": "图片尺寸，如1600x2848,1344x3136,3040x5504等，默认请传入1344x3136",
                    },
                    "image": {
                        "type": ["string", "array"],
