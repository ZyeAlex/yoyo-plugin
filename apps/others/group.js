...
async function increaseAccept(e) {
  // 存储群员信息
  if (members[e.group_id]) members[e.group_id].push?.(Number(e.user_id))
  /** 定义入群欢迎内容 */
...
async function decreaseAccept(e) {
  // 移除群员信息
  if (members[e.group_id]) {
    let index = members[e.group_id].findIndex(qq => qq == Number(e.user_id))
    if (index > -1) members[e.group_id].splice(index, 1)
  }
...
// 群管理
async function manage(e, m1, m2, m3) {
  await utils.checkPermission(e, 'admin', 'admin')
  // 检测管理员权限
  if (e.bot.adapter?.name !== "OneBotv11" || typeof e.bot.sendApi !== "function") return true

  // 获取被权限的成员
  let qqs = e.message.filter(item => item.type === 'at').map(({ qq }) => Number(qq))
  let qstr = m1
  if (qstr) qqs = qqs.concat(Number(qstr.trim().split(/[ ,，]/g)))
  qstr = m3
  if (qstr) qqs = qqs.concat(Number(qstr.trim().split(/[ ,，]/g)))

  // 获取qq群
  let groups = [e.group_id]
  if (e.msg.includes('全') || e.msg.includes('q') || e.msg.includes('Q')) groups = await getGroupList(e)


  // 获取禁言时间
  let time = utils.durationToSeconds(m2 || 60)
...
// 踢出成员
let kiskLogs = setting.getData('data/logs/kick-logs', [])
async function kick(e, groups, qqs, reject_add_request) {
  if (!qqs.length) return
  // 遍历踢出
  let members = await getMemberList(groups)
  for (let group_id of groups) {
    for (let user_id of qqs.filter(qq => members[group_id].includes(qq))) {
      try {
        ...
        // 保存日志 e.user_id  qq
        setting.setData('data/logs/kick-logs',
          [
            {
              time: utils.getDate(),
              admin: e.user_id,
...
async function del(e, groups, qqs) {
  ...
  for (let group_id of groups) {
    ...
    const targetMsgs = messages.reverse().filter(msg => qqs.filter(qq => members[group_id].includes(qq)).find(qq => qq == msg.user_id))
...
// 设置头衔
async function title(e, text) {
  await utils.checkPermission(e, 'all', 'owner')
  const res = await e.group.setTitle(e.user_id, text)
}
...
async function getGroupList(e) {
  ...
  groupList = (await e.bot.getGroupList()).filter(group => {
    let groupObj = Bot.pickGroup(group)
    if (!groupObj || !groupObj.is_admin && !groupObj.is_owner) return false
    return true
  })
  return groupList
}
 ...
 * 获取QQ群内的成员信息
 */
async function getMemberList(groupList) {
  for (let group of groupList) {
    if (members[group]) continue
    try {
      let memberList = await Bot.pickGroup(group).getMemberList()
      members[group] = memberList.map(qq => Number(qq))
    } catch (error) { }
  }
  return members
}
